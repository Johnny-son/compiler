...
但当前前端 AST 还没有完整数组下标表达式节点。因此 `a[i]` 这类表达式还没有在 `IRGenerator` 中生成 GEP。

后续要做：

- 扩展 grammar / CSTVisitor / AST。
- 增加数组声明节点。
- 增加数组元素左值节点。
- `emitLValue(a[i])` 生成 `GetElementPtrInst`。
- `emitRValue(a[i])` 在 GEP 后追加 `LoadInst`。
...
### 5. 错误测试脚本在 macOS bash 下受限

`scripts/run_ci_tests.sh -err --all` 用到了 `mapfile`。

macOS 默认 bash 版本较老，没有 `mapfile`，因此当前环境无法直接跑完整错误测试。

这不是 IR 迁移导致的问题。可以用新版 bash 执行，或者把脚本改成兼容 POSIX/sh 的读数组方式。
...
5. 清理 `Value`、`FormalParam`、`GlobalVariable` 中旧后端寄存器字段。
