...
- `scripts/run_ci_tests.sh -ir --all --compiler ./build-local/compiler --show-failures` 通过，结果 `OK number=30, NG number=0`。
...
- `scripts/run_ci_tests.sh -err --all` 在 macOS 自带 bash 下因缺少 `mapfile` 无法运行，不是本轮 IR 代码错误。
